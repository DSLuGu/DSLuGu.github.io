...
- [점과 평면 사이의 거리]
    ...
    > $D=[\lVert proj_n{v} \rVert]=$  
    > $\frac{\lVert (x_0-x,y_0-y,z_0-z) \cdot (a,b,c) \rVert}{(\lVert n \rVert)^2} \lVert n \rVert = \frac{lVert a(x_0-x)+b(y_0-y)+c(z_0-z) \rVert}{\lVert n \rVert} = \frac{|a x_0 + b y_0 + c z_0 - (ax+by+cz)}{\sqrt{a^2+b^2+c^2}} = \frac{|a x_0 + b y_0 + c z_0 - (-d)}{\sqrt{a^2+b^2+c^2}} = \frac{|a x_0 + b y_0 + c z_0 + d}{\sqrt{a^2+b^2+c^2}}$
